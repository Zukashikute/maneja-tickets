...
const GoogleUserUpdate= async (req, res) => {
    ...
    //  #swagger.description = 'Updates an existing userame in the database
    ...
    try {
        ...
        const result = await google_users.findByIdAndUpdate(_id, updates, { new: true });
        if (!result) {
            return res
                .status(404)
...
const GoogleUserDelete = async (req, res) => {
    ...
    try {
        ...
        const result = await google_users.deleteOne({ _id: _id }).then((data) => {
